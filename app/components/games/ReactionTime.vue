...
<template>
  <div class="flex flex-col gap-4 p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
    <div class="flex items-center justify-between">
      ...
      <span 
        v-if="bestTime < 999999" 
        class="text-sm text-MyGreen font-mono"
      >
        Record: {{ bestTime }}ms
      </span>
    </div>
    ...
    <button
      class="h-40 rounded-xl flex flex-col items-center justify-center transition-colors duration-200
    ...
    >
      ...
      >
        {{ reactionTime < 200 ? '🔥 Excellent !' : reactionTime < 300 ? '👍 Bien !' : reactionTime < 400 ? '😊 Pas mal' : '🐢 Tu peux mieux faire' }}
      </span>
    </button>
