  ...
  <div class="flex flex-col gap-4 p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-3 text-sm text-zinc-400 font-mono">
        <span v-if="game.active" class="text-MyYellow">⏱️ {{ game.timeLeft }}s</span>
        <span v-if="game.active">🎯 {{ game.score }}pts</span>
        <span v-if="game.combo > 1" class="text-MyGreen">🔥 x{{ game.combo }}</span>
      </div>
    </div>

    <!-- Zone de jeu -->
    <div 
      class="relative h-64 rounded-xl bg-gradient-to-b from-zinc-900 to-zinc-800 
    ...
    >
      ...
      <button
        v-for="star in stars"
        :key="star.id"
        class="absolute transition-transform duration-100 cursor-pointer hover:scale-125
      ...
      >
        <p class="text-white font-bricolage text-lg mb-2">
          {{ game.score > 0 ? `Score: ${game.score} pts` : 'Attrape les étoiles !' }}
        </p>
    ...
    <button
      v-if="!game.active"
      class="py-3 rounded-xl bg-MyYellow text-black font-bricolage font-semibold
             hover:bg-MyYellow/80 transition-colors"
      @click="startGame"
    >
