...
const getRandomWord = () =>
  WORDS[Math.floor(Math.random() * WORDS.length)] || "chaos";
...
const handleInput = (e: Event) => {
  ...
  if (
    state.value.userInput.toLowerCase() === state.value.targetWord.toLowerCase()
  ) {
...
<template>
  ...
  >
    ...
    <div class="flex justify-between text-sm text-zinc-400 font-mono">
      <span
        >Mots: <span class="text-MyGreen">{{ state.wordsCompleted }}</span
        >/5</span
      >
      <span
        >Score: <span class="text-MyYellow">{{ state.score }}</span></span
      >
    </div>
    ...
    <input
      ...
      type="text"
      class="w-full px-4 py-3 bg-zinc-950 border border-zinc-700 rounded-xl text-center font-mono text-xl text-white placeholder-zinc-600 focus:outline-none focus:border-MyBlue transition-colors"
    ...
    >
      {{
        state.wordsCompleted >= 5
          ? "✅ Bien joué !"
          : `❌ ${state.wordsCompleted} mots seulement`
      }}
    </p>
  </div>
</template>
