...
const message = ref('Choisis une image et clique sur Mélanger pour commencer !')
...
const handleTileClick = (idx: number) => {
  ...
  // Swap
  ;[tiles.value[idx], tiles.value[emptyIdx]] = [tiles.value[emptyIdx]!, tiles.value[idx]!]
  game.value.moves++
  emit('interaction', 1)
  ...
  if (isSolved(tiles.value)) {
    ...
    const isNewBestTime = game.value.bestTime === 0 || game.value.timer < game.value.bestTime
    const isNewBestMoves = game.value.bestMoves === 0 || game.value.moves < game.value.bestMoves
  ...
  <div class="flex flex-col gap-4 p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
    ...
    <div class="flex justify-center gap-4">
      <button
        ...
        :class="selectedSvg === svg ? 'border-MyPink scale-105' : 'border-zinc-600 opacity-60 hover:opacity-100'"
        @click="selectSvg(svg)"
      >
    ...
    <!-- Records -->
    <div v-if="game.bestTime > 0" class="flex justify-center gap-6 text-xs text-MyGreen font-mono">
    ...
    <!-- Bouton mélanger -->
    <button
      class="py-3 rounded-xl bg-MyCyan text-black font-bricolage font-semibold
             hover:bg-MyCyan/80 transition-colors"
      @click="shuffle"
    >
