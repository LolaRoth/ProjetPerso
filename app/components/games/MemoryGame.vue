...
const emit = defineEmits<{
  complete: [won: boolean, level: number, moves: number, time: number, maxCombo: number]
  ...
  if (flippedCards.value.length === 2) {
    ...
    if (firstCard && secondCard && firstCard.pairId === secondCard.pairId) {
      ...
      if (cards.value.every(c => c.matched)) {
        ...
        emit('complete', true, level.value, moves.value, timer.value, maxCombo.value)
      }
    } else {
...
<template>
  <div class="flex flex-col gap-4 p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
    ...
    <div v-if="!gameActive && cards.length === 0" class="flex justify-center gap-3">
      <button
        v-for="lvl in [1, 2, 3]"
        :key="lvl"
    ...
    >
      <button
        v-for="card in cards"
        :key="card.id"
        class="aspect-square rounded-lg text-2xl transition-all duration-300 
    ...
    <button
      v-if="!gameActive"
      class="py-3 rounded-xl bg-MyPink text-black font-bricolage font-semibold
             hover:bg-MyPink/80 transition-colors"
      @click="initGame()"
    >
