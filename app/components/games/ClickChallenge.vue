...
const progressText = computed(() => {
  ...
  if (!state.value.started) return `Objectif: ${state.value.target} clics en 5s`
...
<template>
  <div class="flex flex-col items-center gap-4 p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
    ...
    <div class="w-full h-3 bg-zinc-800 rounded-full overflow-hidden">
      <div 
        ...
        :class="progressColor"
        :style="{ width: `${Math.min(100, (state.current / state.target) * 100)}%` }"
      />
    </div>
    ...
    <button
      ...
      @click="handleClick"
    >
      {{ state.active ? (state.started ? '👆 CLIQUE !' : '▶️ Commencer') : '🎮 Jouer' }}
    </button>
