...
const emit = defineEmits<{
  complete: [
    score: number,
    stats: { hits: number; missed: number; maxCombo: number; accuracy: number },
  ];
...
const accuracy = computed(() => {
  ...
  return Math.round(
    (gameState.value.hits / gameState.value.totalSpawned) * 100,
  );
});
...
const hitTarget = (id: number) => {
  ...
  const timeBonus = Math.max(
    0,
    Math.floor((target.lifetime - timeAlive) / 100),
  );
  const comboBonus = Math.min(gameState.value.combo * 2, 20);
  const points = Math.round(
    (10 + comboBonus + timeBonus) * settings.value.multiplier,
  );
  ...
  >
    ...
    >
      ...
      <button
        ...
        :key="target.id"
        class="absolute rounded-full bg-gradient-to-br from-MyPink to-MyPink/60 border-2 border-white/30 cursor-crosshair transition-transform hover:scale-110 active:scale-90"
        :style="{
          left: `${target.x}%`,
          top: `${target.y}%`,
