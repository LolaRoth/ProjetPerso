...
<template>
  ...
  >
    ...
    <!-- Pièces disponibles -->
    <div
      class="flex flex-wrap gap-3 justify-center p-4 bg-zinc-800/50 rounded-xl min-h-[80px]"
    >
      <div
        v-for="piece in unplacedPieces"
        :key="piece.id"
        draggable="true"
        class="flex items-center justify-center cursor-grab active:cursor-grabbing transition-all hover:scale-110 select-none"
    ...
    <!-- Bouton reset -->
    <button
      class="py-2 rounded-lg bg-zinc-700 text-white font-bricolage text-sm hover:bg-zinc-600 transition-colors"
      @click="resetPuzzle"
    >
      🔄 Nouveau Puzzle
    </button>
